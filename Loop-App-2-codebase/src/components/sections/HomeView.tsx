...
export default function HomeView() {
  ...
  return (
    <div className="space-y-3 pt-1">
      {/* Top Bar with Expected Fares Pill on Left */}
      <div className="flex items-center justify-between gap-2 px-0.5 pb-0.5">
        <button
          onClick={() => setShowFaresModal(true)}
          className="h-7 px-3 rounded-full bg-[#FFC554]/15 border border-[#FFC554]/30 text-[#FFC554] font-black text-[10px] uppercase tracking-wider flex items-center gap-1.5 active:scale-95 shadow-sm hover:bg-[#FFC554]/25 transition-all shrink-0"
        >
          <span className="font-extrabold text-xs">₹</span>
          <span>Expected Fares</span>
        </button>

        {activeLoops.filter(l => l.status === 'open').length > 0 && (
          <span className={`text-[10px] font-bold ${mutedText} uppercase tracking-wider`}>
            {activeLoops.filter(l => l.status === 'open').length} Active {activeLoops.filter(l => l.status === 'open').length === 1 ? 'Ride' : 'Rides'}
          </span>
        )}
      </div>
